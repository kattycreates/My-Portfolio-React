export const slidesData = [
    {
        id: 1,
        title: 'B-logger',
        img: 'assets/b-logger.png',
        demo: 'https://b-logger.netlify.app/',
        github: 'https://github.com/kattycreates/b-logger'

      },
    {
        id: 2,
        title: 'To-do list',
        img: 'assets/to-do-list.png',
        demo: 'https://to-do-list-by-katty.netlify.app/',
        github: 'https://github.com/kattycreates/to-do-list'

      },
       {
        id: 3,
        title: 'Weather App',
        img: 'assets/weather-app.png',
        demo: 'https://kattycreates.github.io/weather-app/',
        github: 'https://github.com/kattycreates/weather-app'
      },
      {
        id: 4,
        title: 'Random Quote Generator',
        img: 'assets/quote-machine.png',
        demo: 'https://kattycreates.github.io/Random_Quote_Machine/',
        github: 'https://github.com/kattycreates/Random_Quote_Machine'
      }, 
    {
      id: 5,
      title: 'Markdown Previewer',
      img: 'assets/markdown-previewer.png',
      demo: 'https://kattycreates.github.io/Markdown_previewer/',
      github: 'https://github.com/kattycreates/Markdown_previewer'

    },  {
      id: 6,
      title: 'Pricing Component',
      img: 'assets/pricing-component.png',
      demo: 'https://kattycreates.github.io/Pricing_component/',
      github: 'https://github.com/kattycreates/Pricing_component'
    }
    /* {
      id: 6,
      title: 'dolorem quibusdam quasi',
      label: 'Adipisci officiis repudiandae.',
      img: '',
      demo: '',
      github: ''
    } */
  ];

export const softSkills=[{id:21,skill:"open to learning",icon:"fas fa-book-reader"},
{id:22,skill:"client focused",icon:"fas fa-users"},
{id:23,skill:"ability to communicate",icon:"fas fa-comments"},
{id:24,skill:" attention to details",icon:"fas fa-search"},
{id:25,skill:"solution oriented",icon:"fas fa-lightbulb"},
{id:26,skill:"empathy",icon:"fas fa-hand-holding-heart"},
{id:27,skill:"acceptable & flexible",icon:"fas fa-vote-yea"},
{id:28,skill:"adaptable",icon:"fas fa-hands-helping"}];



export const techSkills=[{id:11,skill:"HTML",icon:"fab fa-html5"},
{id:12,skill:"CSS",icon:"fab fa-css3-alt"},
{id:13,skill:"Bootstrap",icon:"fab fa-bootstrap"},
{id:14,skill:"JavaScript",icon:"fab fa-js-square"},
{id:15,skill:"React",icon:"fab fa-react"},
{id:16,skill:"Git",icon:"fab fa-git"},
{id:17,skill:"GitHub",icon:"fab fa-github"}];


